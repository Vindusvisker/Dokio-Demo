...
// Demo goals data for showcase
const DEMO_GOALS: FinancialGoal[] = [
  {
    id: 'camping-trip-2024',
    name: 'Epic Camping Adventure',
    description: 'A week-long camping trip with friends in the Norwegian mountains. Complete with hiking, fishing, and campfire stories under the stars.',
    targetAmount: 8500, // NOK
    savedAmount: 6200, // 73% progress - nearly there!
    category: 'experience',
    priority: 'high',
    deadline: '2024-07-15',
    imageUrl: 'https://parade.com/.image/t_share/MjA2ODA1NjM1MzQwMTgzMDEx/camping-instagram-captions.jpg',
    externalUrl: 'https://www.visitnorway.com/things-to-do/outdoor-activities/hiking/'
  }
];

// Demo state for realistic showcase
const DEMO_SAVINGS_STATE = {
  totalSaved: 18750, // Total savings amount in NOK
  snusSavings: 16250, // From reduced consumption
  monthlyContributions: 2500, // Additional monthly savings
  daysTracking: 127, // Days since starting
  baselineSnusPerDay: 15, // Original consumption
  currentSnusPerDay: 8 // Current reduced consumption
};

export default function GoalTracker() {
  const [goals, setGoals] = useState<FinancialGoal[]>(DEMO_GOALS)
  const [totalSaved, setTotalSaved] = useState(DEMO_SAVINGS_STATE.totalSaved)
  const [loading, setLoading] = useState(false) // No loading for demo data
  const [showAddDialog, setShowAddDialog] = useState(false)
  const [editingGoal, setEditingGoal] = useState<FinancialGoal | null>(null)
  const [userConfig, setUserConfig] = useState<UserConfig | null>(null)
  const [userConfigLoading, setUserConfigLoading] = useState(false) // No loading for demo

  // Demo initialization
  useEffect(() => {
    // Set up demo user config to match the main app
    ...
    setUserConfig(demoUserConfig);
    
    // Distribute demo savings across goals
    const distributedGoals = distributeSavingsToGoals(DEMO_GOALS, DEMO_SAVINGS_STATE.totalSaved);
    setGoals(distributedGoals);

    console.log('🎯 GoalTracker initialized with demo data:');
    console.log('- Total saved:', `${DEMO_SAVINGS_STATE.totalSaved} NOK`);
    console.log('- Goals:', DEMO_GOALS.map(g => g.name).join(', '));
    console.log('- Camping trip progress:', `${Math.round((6200/8500)*100)}%`);
  }, []);
  ...
  // Demo version - no storage calls needed
  const loadGoalsAndSavings = async () => {
    console.log('🔄 Demo: Refreshing goals display');
    const distributedGoals = distributeSavingsToGoals(goals, totalSaved);
    setGoals(distributedGoals);
  }

  // Demo calculation methods (no storage access)
  const calculateTotalSavings = async (): Promise<number> => {
    return DEMO_SAVINGS_STATE.totalSaved;
  }

  const calculateSnusSavings = async (): Promise<number> => {
    return DEMO_SAVINGS_STATE.snusSavings;
  }

  const calculateMonthlyContributions = async (): Promise<number> => {
    return DEMO_SAVINGS_STATE.monthlyContributions;
  }
  ...
  const handleGoalAdded = (newGoal: FinancialGoal) => {
    // Demo version: Add to local state
    const newGoals = [...goals, newGoal];
    const distributedGoals = distributeSavingsToGoals(newGoals, totalSaved);
    setGoals(distributedGoals);
    console.log('🎯 Demo: Added new goal:', newGoal.name);
  }
  ...
  const handleGoalUpdated = (updatedGoal: FinancialGoal) => {
    ...
    const distributedGoals = distributeSavingsToGoals(updatedGoals, totalSaved);
    setGoals(distributedGoals);
    setEditingGoal(null);
    console.log('🎯 Demo: Updated goal:', updatedGoal.name);
  }
  ...
  const handleDeleteGoal = (goalId: string) => {
    ...
    const distributedGoals = distributeSavingsToGoals(updatedGoals, totalSaved);
    setGoals(distributedGoals);
    console.log('🎯 Demo: Deleted goal with ID:', goalId);
  }
